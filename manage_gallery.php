    ...
    if (isset($_POST['add_gallery'])) {
        ...
        if ($check !== false) {
            if (move_uploaded_file($_FILES["gallery_image"]["tmp_name"], $target_file)) {
                ...
                $stmt = $conn->prepare("INSERT INTO atharv_gallery (event_name, event_date, description, image_path, thumbnail_path) VALUES (?, ?, ?, ?, ?)");
        ...
        $stmt = $conn->prepare("UPDATE atharv_gallery SET is_active = ? WHERE id = ?");
        ...
        $stmt = $conn->prepare("SELECT image_path, thumbnail_path FROM atharv_gallery WHERE id = ?");
        ...
        $stmt = $conn->prepare("DELETE FROM atharv_gallery WHERE id = ?");
        $stmt->bind_param("i", $id);
...
$gallery_items = [];
$stmt = $conn->prepare("SELECT * FROM atharv_gallery ORDER BY event_date DESC");
$stmt->execute();
$result = $stmt->get_result();
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        <?php foreach ($gallery_items as $item): ?>
                                            <tr>
                                                <td><img src="<?php echo $item['thumbnail_path']; ?>" class="thumbnail" alt="Thumbnail"></td>
